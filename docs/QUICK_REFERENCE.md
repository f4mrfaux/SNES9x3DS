...
## What the code actually does (Plan E path)

- Stereo enable: `impl3dsRunOneFrame` reads the hardware slider (`osGet3DSliderState()`), multiplies by user max depth and strength (`StereoSliderValue` × `StereoDepthStrength`), and toggles stereo if effective ≥ 0.01.
- Layer offsets: `stereo3dsUpdateLayerOffsetsFromSlider` builds per-eye offsets from `LayerDepth[]` and `ScreenPlaneLayer` (defaults in `source/3dssettings.h`), writing into `g_stereoLayerOffsets[2][5]`.
- Layer tagging: `source/Snes9x/gfxhw.cpp` sets `g_currentLayerIndex` before each BG0-3 and OBJ draw so offsets apply when vertices are emitted.
- Vertex emission: `gpu3dsAddTileVertexes` (`source/3dsimpl_gpu.h`) writes per-eye vertex lists, applying the current layer’s offset.
- Drawing: `gpu3dsDrawVertexes` (`source/3dsimpl_gpu.cpp`) renders both eyes to per-eye textures if stereo is enabled (skips right eye when slider≈0), otherwise the legacy mono target. Each eye is cleared (color+depth) before draw, depth test/write enabled.
- Transfer: `stereo3dsTransferToScreenBuffers` (`source/3dsstereo.cpp`) copies left/right textures to `GFX_LEFT`/`GFX_RIGHT`; mono fallback uses `gpu3dsTransferToScreenBuffer`.

## Known gaps vs docs

- Mode7 depth is experimental: per-eye offsets applied to Mode7 line vertices (treated as BG0); optional gradient using `Mode7DepthNear/Far` and `Mode7UseGradient`; no per-sprite depth yet.
- Depth format is DEPTH24_STENCIL8 via `GPU_SetViewport`, but depth buffers are allocated as RGBA8 blocks (4 B/px) for storage; clears happen per-eye and per-frame.

## Quick reference (current pipeline)

| Step | Where | Notes |
|------|-------|-------|
| Slider read & enable | `impl3dsRunOneFrame` | Uses hardware slider × user depth × strength; threshold 0.01 |
| Offset build | `stereo3dsUpdateLayerOffsetsFromSlider` | Anchors to `ScreenPlaneLayer`; clamps per-eye shift |
| Layer tagging | `gfxhw.cpp` macros (BG0-3, OBJ) | Sets `g_currentLayerIndex` before each layer draw |
| Vertex emit | `gpu3dsAddTileVertexes` | Writes per-eye buffers with horizontal offsets |
| Eye draw | `gpu3dsDrawVertexes` | Renders each eye to its texture; UI rectangles stay mono |
| Transfer | `stereo3dsTransferToScreenBuffers` | Copies eye textures to GFX_LEFT/GFX_RIGHT; mono fallback available |

## Parallax sign convention

- Positive depth → into screen → left eye shifts left, right eye shifts right.
- Negative depth → pop-out → left eye shifts left, right eye shifts right with opposite sign (handled by `g_stereoLayerOffsets` sign flip).
